...
public class BowShootActionParser extends PlayerActionParser {
  private final PlayerShootBowEvent event;

  public BowShootActionParser(final PlayerShootBowEvent event, final Player argument, final String action, final UnderscoreEnchants plugin) {
  ...
  public void execute() {
    ...
    final BowShootActions action = BowShootActions.lookup(getAction().split(" ")[0]);
  ...
  public String[] parsePlaceholders(String args, final Player player, final UnderscoreEnchants plugin) {
    ...
    for (final String str : args.split("[(\\)\\+\\*\\-\\/ ]")) {
      switch (BowShootActionPlaceholders.lookup(str)) {
        case null -> {}
        case FORCE -> args = args.replace(str, Float.toString(event.getForce()));
      }
    }
    return args.split(" ");
  }
