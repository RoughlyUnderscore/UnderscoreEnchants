package com.roughlyunderscore.enchs.parsers.action.events.armor;

import lombok.ToString;

@ToString
public enum ArmorEquipActionPlaceholders {
  ;

  private final String[] aliases;

  ArmorEquipActionPlaceholders(final String... aliases0) {
    String[] aliases = new String[aliases0.length];
    for (int i = 0; i < aliases0.length; i++) aliases[i] = aliases0[i].toUpperCase();

    this.aliases = aliases;
  }

  ArmorEquipActionPlaceholders() {
    this.aliases = new String[] {this.name()};
  }


  public boolean matches(final String arg) {
    for (String name : aliases) if (name.equals(arg.toUpperCase())) return true;
    return false;
  }

  public static ArmorEquipActionPlaceholders lookup(final String arg) {
    for (ArmorEquipActionPlaceholders condition : values()) if (condition.matches(arg)) return condition;
    return null;
  }
}
