...
public class BowHitActionParser extends DoublePlayerActionParser {
  private final PlayerBowHitEvent event;

  public BowHitActionParser(final PlayerBowHitEvent event, final Player damager, final Player victim, final String action, final UnderscoreEnchants plugin) {
  ...
  public void execute() {
    ...
    final BowHitActions action = BowHitActions.lookup(getAction().split(" ")[0]);
  ...
  public String[] parsePlaceholders(String args, final Player first, final Player second, final UnderscoreEnchants plugin) {
    for (final String str : args.split(" ")) {
      switch (BowHitActionPlaceholders.lookup(str)) {
        case null -> {}
