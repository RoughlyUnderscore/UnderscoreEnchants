...
/**
 * Sets player's current experience, clamping it between 0 and Int.MAX_VALUE
 *
 * Example:
 * - `exp 15`
 *
...
@Since("2.2")
@Stable
class PlayerSetExpAction : RegistrableAction {
  override val aliases = listOf(
    "exp", "set-exp", "current-exp", "set-current-exp", "xp", "set-xp", "current-xp", "set-current-xp"
  )
  ...
  override fun execute(event: Event, trigger: RegistrableTrigger, arguments: List<String>, target: TargetType): EventModifications? {
    ...
    val exp = arguments[0].toDoubleOrNull()?.roundToInt() ?: return null
    ...
    player.level = 0
    player.exp = 0f
    player.giveExp(exp clamp 0 .. Int.MAX_VALUE)
