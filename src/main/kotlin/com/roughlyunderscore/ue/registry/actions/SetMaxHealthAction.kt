...
/**
 * Sets entity's max health, clamping it between 0 and Double.MAX_VALUE
 *
 * Example:
 * - `max-hp 306`
 *
...
@Since("2.2")
@Stable
class SetMaxHealthAction : RegistrableAction {
  override val aliases = listOf("max-hp", "set-max-hp", "max-health", "set-max-health")
  ...
  override fun execute(event: Event, trigger: RegistrableTrigger, arguments: List<String>, target: TargetType): EventModifications? {
    ...
    entity.getAttribute(Attribute.GENERIC_MAX_HEALTH)?.baseValue = health.clamp(0.0, 2048.0)
