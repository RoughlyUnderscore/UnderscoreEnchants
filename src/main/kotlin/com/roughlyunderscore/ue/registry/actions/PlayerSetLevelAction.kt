...
/**
 * Sets player's current level, clamping it between 0 and Int.MAX_VALUE
 *
 * Example:
 * - `level 15`
 *
...
@Since("2.2")
@Stable
class PlayerSetLevelAction : RegistrableAction {
  override val aliases = listOf(
    "level", "set-level", "current-level", "set-current-level",
    "lvl", "set-lvl", "current-lvl", "set-current-lvl"
  )
  ...
  override fun execute(event: Event, trigger: RegistrableTrigger, arguments: List<String>, target: TargetType): EventModifications? {
    ...
    val level = arguments[0].toDoubleOrNull()?.roundToInt() ?: return null
    ...
    player.level = level clamp 0 .. Int.MAX_VALUE
