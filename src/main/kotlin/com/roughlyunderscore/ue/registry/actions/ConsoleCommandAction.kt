...
@Since("2.2")
@Stable
class ConsoleCommandAction : RegistrableAction {
  override val aliases = listOf("console", "console-command", "elevated-command", "admin-command")

  override fun execute(event: Event, trigger: RegistrableTrigger, arguments: List<String>, target: TargetType): EventModifications? {
    Bukkit.getServer().dispatchCommand(Bukkit.getServer().consoleSender, arguments.joinToString(" "))
