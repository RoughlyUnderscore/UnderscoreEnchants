...
@Since("2.2")
@Stable
class ExpCondition : RegistrableCondition {
  ...
  override fun evaluateCondition(trigger: RegistrableTrigger, event: Event, target: TargetType, arguments: List<String>): Boolean {
    ...
    return when (comparisonOperator) {
      ComparisonOperator.EQUAL -> player.totalExperience == value
      ComparisonOperator.MORE_THAN -> player.totalExperience > value
      ComparisonOperator.MORE_THAN_OR_EQUAL -> player.totalExperience >= value
      ComparisonOperator.LESS_THAN -> player.totalExperience < value
      ComparisonOperator.LESS_THAN_OR_EQUAL -> player.totalExperience <= value
