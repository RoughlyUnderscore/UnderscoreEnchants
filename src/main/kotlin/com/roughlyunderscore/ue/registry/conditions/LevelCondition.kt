...
@Since("2.2")
@Stable
class LevelCondition : RegistrableCondition {
  ...
  override fun evaluateCondition(trigger: RegistrableTrigger, event: Event, target: TargetType, arguments: List<String>): Boolean {
    ...
    return when (comparisonOperator) {
      ComparisonOperator.EQUAL -> player.level == value
      ComparisonOperator.MORE_THAN -> player.level > value
      ComparisonOperator.MORE_THAN_OR_EQUAL -> player.level >= value
      ComparisonOperator.LESS_THAN -> player.level < value
      ComparisonOperator.LESS_THAN_OR_EQUAL -> player.level <= value
